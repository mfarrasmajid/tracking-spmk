pipeline {
    ...
    environment {
        ...
        TARGET_PATH_CHILD = 'trackingspmk'
        TARGET_PATH = '/var/www/trackingspmk'
        SONAR_TOKEN = credentials('sonarqube-login-farras')
        REPORT_NAME = 'dast-report-tracking-spmk.html'
        TRIVY_NAME = 'trivy-tracking-spmk.html'
        TRIVY_FS_NAME = 'trivy-fs-tracking-spmk.html'
        K6_NAME = 'load-performance-report-tracking-spmk.html'
        K6_PROD_NAME = 'load-performance-report-production-tracking-spmk.html'
        DB_DEV_HOST = credentials('db-dev-host')
        DB_DEV_USER = credentials('db-dev-user')
        DB_DEV_PASSWORD = credentials('db-dev-password')
        DB_DEV_PORT = credentials('db-dev-port')
        VM_DEV_USER = credentials('vm-dev-user')
        VM_DEV_PASSWORD = credentials('vm-dev-password')
        VM_DEV_HOST = credentials('vm-dev-host')
        VM_DEV_PATH = '/var/www/html/trackingspmk'
        VM_PROD_USER = credentials('vm-tracking-spmk-user')
        VM_PROD_PASSWORD = credentials('vm-tracking-spmk-password')
        VM_PROD_HOST = credentials('vm-tracking-spmk-host')
        VM_PROD_PATH = '/var/www/html/trackingspmk'
        DEV_URL = 'https://dev.mitratel.co.id/trackingspmk'
        PROD_URL = 'https://digital.mitratel.co.id/trackingspmk'
    }
