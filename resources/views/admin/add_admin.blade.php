...
@if (isset($data['id']))
@section('menu', 'Edit Admin')
@section('sub_menu', 'Ubah Data Admin')
@else 
@section('menu', 'Add Admin')
@section('sub_menu', 'Tambahkan Admin Baru')
@endif
...
<div class="card card-custom">
    ...
    <!--begin::Form-->
    <form class="form" method="POST" @if(isset($data['id'])) action="{{url('/admin/add_admin').'/'.$data['id']}}" @else action="{{ url('/admin/add_admin') }}" @endif>
    @csrf
        <div class="card-body">
            @if (session('error'))
            ...
            <div class="form-group mb-5">
                ...
                <input type="text" class="form-control" name="nik_tg" @if(isset($data['id'])) value="{{$data['admin']->nik_tg}}" @endif required/>
            </div>
            <div class="form-group mb-5">
                <label class="mb-2">Name</label>
                <input type="text" class="form-control" name="name" @if(isset($data['id'])) value="{{$data['admin']->name}}" @endif required/>
            </div>
            <div class="form-group mb-5">
                <label class="mb-2">Email</label>
                <input type="text" class="form-control" name="email" @if(isset($data['id'])) value="{{$data['admin']->email}}" @endif required/>
            </div>
        </div>
        <div class="card-footer">
            <button type="submit" class="btn btn-danger mr-2">Submit</button>
            <a href="{{url('/admin/manage_admin')}}" class="btn btn-secondary">Back</a>
        </div>
    </form>
    <!--end::Form-->
</div>
