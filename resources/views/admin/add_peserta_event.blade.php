...
@section('title', 'Add Peserta Event')

@if (isset($data['id']))
@section('menu', 'Edit Peserta Event')
@section('sub_menu', 'Ubah Data Peserta Event')
@else 
@section('menu', 'Add Peserta Event')
@section('sub_menu', 'Tambahkan Peserta Event Baru')
@endif
...
<div class="card card-custom">
    ...
    <h3 class="card-title">
    @if (isset($data['id']))
    Edit Peserta Event ID {{$data['id']}}
    @else
    Input Peserta Event Baru
    @endif
    </h3>
    </div>
    <!--begin::Form-->
    <form class="form" enctype="multipart/form-data" method="POST" @if(isset($data['id'])) action="{{url('/admin/add_peserta_event').'/'.$data['id']}}" @else action="{{ url('/admin/add_peserta_event') }}" @endif>
    @csrf
        <div class="card-body">
            @if (session('error'))
            ...
            <div class="form-group mb-5">
                <label class="mb-2">Event</label>
                <select name="event_id" class="form-control">
                    @foreach($data['event'] as $event)
                    <option value="{{$event->id}}" @if(isset($data['id'])) @if($event->id == $data['peserta_event']->event_id) selected @endif @endif>{{$event->event}}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group mb-5">
                <label class="mb-2">NIK TG</label>
                <input type="text" class="form-control" name="nik_tg" @if(isset($data['id'])) value="{{$data['peserta_event']->nik_tg}}" @endif required/>
            </div>
            <div class="form-group mb-5">
                <label class="mb-2">Name</label>
                <input type="text" class="form-control" name="name" @if(isset($data['id'])) value="{{$data['peserta_event']->name}}" @endif/>
            </div>
            <div class="form-group mb-5">
                <label class="mb-2">Posisi</label>
                <input type="text" class="form-control" name="posisi" @if(isset($data['id'])) value="{{$data['peserta_event']->posisi}}" @endif required/>
            </div>
            <div class="form-group mb-5">
                <label class="mb-2">Company</label>
                <input type="text" class="form-control" name="company" @if(isset($data['id'])) value="{{$data['peserta_event']->company}}" @endif required/>
            </div>
            <div class="form-group mb-5">
                <label class="mb-2">Prioritas Urutan</label>
                @foreach($data['event_priority'] as $ep)
                <div class="form-check form-check-custom form-check-solid mb-2">
                    <input class="form-check-input" type="radio" name="priority" @if (isset($data['id'])) @if($data['peserta_event']->priority == $ep->priority) checked="checked" @endif @endif value="{{$ep->priority}}" />
            ...
            <div class="form-group mb-5">
                ...
                <input type="text" class="form-control" name="priority_number" @if(isset($data['id'])) value="{{$data['peserta_event']->priority_number}}" @endif required/>
            </div>
            ...
            @elseif ($data['peserta_event']->photo != NULL)
            <input class="replace_name" type="text" hidden name="replace_name" value="{{$data['peserta_event']->photo}}">
            @else
            <input class="replace_name" type="text" hidden name="replace_name" value="">
            @endif
            <div class="form-group mb-5">
                ...
                <div class="image-input image-input-outline" data-kt-image-input="true" style="background-image: url({{ asset('assets/media/svg/avatars/blank.svg')}})">
                    ...
                    @elseif ($data['peserta_event']->photo != NULL)
                    <div class="image-input-wrapper w-125px h-125px" style="background-image: url({{ asset('eviden')}}/{{$data['peserta_event']->photo}})"></div>
                    @else
                    <div class="image-input-wrapper w-125px h-125px" style="background-image: url({{ asset('assets/media/svg/avatars/blank.svg')}})"></div>
                    @endif
        ...
        <div class="card-footer">
            ...
            <a href="{{url('/admin/manage_peserta_event')}}" class="btn btn-secondary">Back</a>
        </div>
    </form>
    <!--end::Form-->
</div>
